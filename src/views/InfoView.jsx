...
import { Crown, Swords, Trophy, Cat, ChevronRight, Shuffle, Loader2 } from 'lucide-react';
...
export default function InfoView({ setActiveStep = () => {}, currentUser }) {
  ...
  // 背景哨兵監聽 (解決 GitHub COOP 錯誤)
  useEffect(() => {
    if (isLoggingIn && currentUser) {
      setIsLoggingIn(false);
      setActiveStep('register');
    }
  }, [currentUser, isLoggingIn, setActiveStep]);
  ...
  const handleAction = async () => {
    ...
    try {
      signInWithPopup(auth, googleProvider).catch((error) => {
        // 如果是使用者自己按叉叉關掉視窗，就停止轉圈圈
        if (error.code === 'auth/popup-closed-by-user') {
          setIsLoggingIn(false);
        } else {
          console.warn("被瀏覽器或 GitHub 政策攔截 (COOP)，交由背景哨兵接手:", error.code);
        }
      });
    } catch (error) {
      ...
      <div className="py-16 text-center">
        ...
        >
          ...
          <span className="tracking-widest drop-shadow-md">
            {currentUser ? '進入報名系統' : (isLoggingIn ? '驗證中...' : '我準備好了，我要報名！')}
          </span>

          {isLoggingIn ? (
            <Loader2 className="w-8 h-8 md:w-10 md:h-10 ml-3 animate-spin text-yellow-200" />
          ) : (
